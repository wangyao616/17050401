...
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
public class nosixteen extends JFrame implements ActionListener{
	public nosixteen() {
		super("计算(非16位)");
	...
	public JPanel getJButton() {
		...
		class myMonitor implements ActionListener {     //事件监听器（点击按钮，触发的事件）
			...
			public void actionPerformed(ActionEvent e) {
				...
				num5.setText("" + (n3+n2*0.03*Math.pow(0.72,(n1-1974))));
			}
	}
		...
		jP.add(jButton2);
		jButton2.addActionListener(this);//***加入事件监听
	...
	public static void main(String[] args) {
		nosixteen nosixteen = new nosixteen();
	}
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand() == "返回") {
			select s = new select(); // 连接16位字长
			dispose();
			}
	}
}
